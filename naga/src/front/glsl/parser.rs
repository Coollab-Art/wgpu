        ...
        loop {
            ...
            match res.kind {
                ...
                LexerResultKind::Directive(directive) => {
                    self.handle_directive(frontend, directive, res.meta)
                }
                LexerResultKind::Error(error) => self.push_preamble_error(frontend, error, res.meta),
            }
        }
    }

    /// Both parse phases lex the whole source, so every preprocessor directive
    /// and lexer error is seen twice. Only the signature-collecting phase acts
    /// on them: the version/profile/extension state a directive sets lives on
    /// `Frontend`, which persists into the body-parsing phase, so re-handling
    /// there would only re-report the same errors.
    fn handles_preamble(&self) -> bool {
        self.phase == ParsePhase::CollectSignatures
    }

    fn handle_directive(&self, frontend: &mut Frontend, directive: Directive, meta: Span) {
        if self.handles_preamble() {
            frontend.handle_directive(directive, meta);
        }
    }

    fn push_preamble_error(&self, frontend: &mut Frontend, error: PreprocessorError, meta: Span) {
        if self.handles_preamble() {
            ...
            match self.lexer.peek()?.kind {
                ...
                LexerResultKind::Error(_) | LexerResultKind::Directive(_) => {
                    ...
                    match res.kind {
                        ...
                        LexerResultKind::Error(error) => {
                            self.push_preamble_error(frontend, error, res.meta)
                        }
